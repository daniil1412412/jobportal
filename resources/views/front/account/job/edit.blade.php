    ...
    @csrf
                ...
                <div class="card border-0 shadow mb-4 ">
                    <div class="card-body card-form p-4">
                        ...
                        <div class="mb-4">
                            <label for="" class="mb-2">Expirience</label>
                            <select class="form-control" name="exp" id="exp" cols="5" rows="5" placeholder="Qualifications">
                               <option value="1" {{ ($job->exp == 1) ? 'selected' : ''}}>1 год</option>
                               <option value="2"{{ ($job->exp == 2) ? 'selected' : ''}}>2 года</option>
                               <option value="3"{{ ($job->exp == 3) ? 'selected' : ''}}>3 года</option>
                               <option value="4"{{ ($job->exp == 4) ? 'selected' : ''}}>4 года</option>
                               <option value="5"{{ ($job->exp == 5) ? 'selected' : ''}}>5 лет</option>
                               <option value="6"{{ ($job->exp == 6) ? 'selected' : ''}}>6 лет</option>
                               <option value="7"{{ ($job->exp == 7) ? 'selected' : ''}}>7 лет</option>
                               <option value="8"{{ ($job->exp == 8) ? 'selected' : ''}}>8 лет</option>
                               <option value="9"{{ ($job->exp == 9) ? 'selected' : ''}}>9 лет</option>
                               <option value="10"{{ ($job->exp == 10) ? 'selected' : ''}}>10 лет</option>
                               <option value="10_plus" {{ ($job->exp == '10_plus') ? 'selected' : ''}}>10+ лет</option>     
                            </select>
                            <p></p>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input value="{{$job->company_web}}" type="text" placeholder="Website" id="company_web" name="company_web" class="form-control">
                            <p></p>
                        </div>
                    </div> 
            ...
            $.ajax({
                ...
                success:function(response){
                    ...
                    if(response.status == true){
                        ...
                        if(errors.JobTypes){
                            $("#JobTypes").addClass('is-invalid').siblings('p').addClass('invalid-feedback').html(errors.JobTypes)
                        }else{
                            $("#JobTypes").removeClass('is-invalid').siblings('p').removeClass('invalid-feedback').html()
                        }
                        if(errors.vacancy){
